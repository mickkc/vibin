...
class TestSearchBuilder {
    ...
    fun testTitleSearch() = transaction {
        ...
        val op = SearchQueryBuilder.build(input, 1)
    ...
    @Test
    fun testPlaylistSearch() = transaction {
        val input = "p:\"My Favorites\""
        val op = SearchQueryBuilder.build(input, 1)
        assertEquals("TRACK.ID IN (SELECT PLAYLIST_TRACK.TRACK_ID FROM PLAYLIST_TRACK WHERE PLAYLIST_TRACK.PLAYLIST_ID IN (SELECT PLAYLIST.ID FROM PLAYLIST WHERE (LOWER(PLAYLIST.\"name\") LIKE '%my favorites%') AND ((PLAYLIST.\"public\" = TRUE) OR (PLAYLIST.ID IN (SELECT PLAYLIST_COLLABORATOR.PLAYLIST_ID FROM PLAYLIST_COLLABORATOR WHERE PLAYLIST_COLLABORATOR.USER_ID = 1)) OR (PLAYLIST.OWNER_ID = 1))))", op.toString())
    }
    ...
    fun testInvalidQuotes() = transaction {
        ...
        assertThrows<IllegalArgumentException>("Unclosed quotes in query") {
            SearchQueryBuilder.build(input, 1)
        }
        return@transaction
    }
